...
const ProjectCard = ({ projects }: { projects: ProjectItem[] }) => {
  ...
  const displayedProjects = projects.length >= 4 ? projects.slice(0, 4) : projects;
      ...
      <div className="grid sm:grid-cols-2 grid-cols-1 space-y-4 sm:space-x-6">
        {displayedProjects.map((project) => {
          ...
          return (
            ...
            >
              <div className="w-full rounded-lg relative overflow-hidden border border-muted h-44">
                {isLoad ? (
                  <video
                    controls={false}
                    muted
              ...
              <div className="flex items-center justify-between">
                <h3 className="text-muted-foreground py-4 px-2">{project.name}</h3>
