...
export default function MoviesPage() {
  ...
  const averageRating =
    MOVIES.filter((m) => m.watched).reduce((acc, m) => acc + m.rating, 0) /
    watchedCount;
        ...
        <div className="px-6 sm:px-8 py-12">
          <div className="mb-8">
            ...
            <p className="text-sm text-muted-foreground mb-6">
              A curated list of films that have shaped my perspective on
              storytelling and cinema.
            </p>
            <p className="text-sm text-muted-foreground leading-relaxed max-w-2xl">
              Note: This is a living collection of movies I&apos;ve watched or
              plan to watch.{" "}
              <span className="text-white/90">
                Each film here has left a lasting impression or is on my
                must-watch list.
              </span>
            </p>
          </div>
              ...
              >
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Film
                      className={`w-4 h-4 ${movie.watched ? "text-foreground" : "text-muted-foreground"}`}
                    />
